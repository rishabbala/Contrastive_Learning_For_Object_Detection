        ...
        for i in batch:
            ...
            for obj in i[1]['annotation']['object']:
                if type(i[0]).__name__ == "Image":
                    i[0] = to_tensor(i[0])
                l = obj['name']
                if ((not self.val) or self.train) and (l == 'chair' or l == 'motorbike'):
                    continue
...
def set_loader(opt):
    ...
    if opt.dataset == 'voc':
        train_collate = VOC_collate(train=True, val=False)
    ...
    if opt.dataset != 'voc':
        ...
        opt.class_to_idx = dict(itertools.islice(opt.class_to_idx.items(), opt.num_classes))
...
def init_val_test(train_loader, val_loader, mean_loader, criterion, opt):
    ...
    for i, (images, labels) in enumerate(mean_loader):
        ...
        out = criterion.backbone_q(images_in_dist).detach().cpu().numpy()
...
def test(opt):
    ...
    if opt.dataset == 'voc':
        ood_threshold_range = np.arange(0, 0.25, 0.01)
        ...
        ood_threshold_range = np.arange(0.1, 0.4, 0.01)
        for ood_threshold in ood_threshold_range:
            print(ood_threshold)
            num_imgs_valid = 0
            acc = 0
    ...
    plt.xlabel('FPR')
    plt.ylabel('TPR')
    plt.savefig("./results/ROC_"+str(opt.dataset)+"_ranking_depth_5.png")
    plt.clf()
    plt.cla()
